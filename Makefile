.ONESHELL:
.PHONY: isort black jacobus beautify commit clean build upload pypi test test311 version works
...
beautify: isort black jacobus

commit: works
	git add -A;
	conda run -n works pip install 'toml_get>=1.0,<2';
	git commit --allow-empty -m "$$(conda run -n works python -m toml_get --key=project --key=version --default=unknown '--outstring=Version %s' @pyproject.toml)";
...
test:
	...
	conda run -n test311 python -c 'import namings.tests; namings.tests.test()';
	conda run -n test311 pip install mypy;
	conda run -n test311 python -m mypy -p namings;
